...
def plot_pollution_gradient(datafile, year, month, particle, out_path,
    ...
    # # Gradient arrows (relative lengths 0.5–1.5)
    # ax.quiver(grid_lon_mesh[::step, ::step],
    #           grid_lat_mesh[::step, ::step],
    #           u_masked[::step, ::step],
    #           v_masked[::step, ::step],
    #           color="black", scale=40, width=0.004,
    #           alpha=0.7, zorder=3)

    import matplotlib as mpl

    # Color arrows by gradient magnitude (like animation)
...
def animate_pollution_gradient_year(
    ...
    for month, poll, grad_lon, grad_lat, mag, mask in monthly_data:
        ...
        # Generate inference
        summaries[month] = generate_inference(
            u, v, mag, mask,
            region_name="J&K + HP + Punjab + Uttarakhand + Haryana",
            grid_lon=grid_lon_mesh,
            grid_lat=grid_lat_mesh,
